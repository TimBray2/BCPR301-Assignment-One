...
class CheckInput:
    ...
    @staticmethod
    def __rearrange(split_data):
        return [split_data[2], split_data[1], split_data[0]]

    def _check_data(self, data, location):
                        ...
                        if count == 6:
                            ...
                            if len(split_data) < 2:
                                ...
                                if location == "database":
                                    split_data = self.__rearrange(split_data)
                                if self.__check_date_validity(split_data):
                                    self.__check = self.__compare_age_to_year(row[2], split_data)
                                else:
                                    self.__check = False
                        else:
