    ...
    def insert_into_database(self, input_list):
        ...
        for row in input_list:
            try:
                ...
                # noinspection PyPep8
                c.execute("INSERT INTO Employee VALUES ('" + str(row[0]) + "', '" + str(row[1]) + "', " + str(
                    row[2]) + ", " + str(row[3]) + ", '" + str(row[4]) + "', " + str(
                    row[5]) + ", '" + split_data + "');")
            except sqlite3.IntegrityError:
                print("Entry is already inside database, this row will not be inserted into the database.")
            except sqlite3.OperationalError:
                print("Invalid token, this row will not be inserted into the database.")
