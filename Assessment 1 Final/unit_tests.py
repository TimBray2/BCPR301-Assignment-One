...
class MainTests(unittest.TestCase):
    ...
    def test_invalid_data_file(self):
        with self.captured_output() as (out, err):
            self.__con.load("file TestFile1.txt")
            self.__con.validate("")
    ...
    def test_valid_data_file(self):
        with self.captured_output() as (out, err):
            self.__con.load("file validInputData.txt")
            self.__con.validate("")
        output = out.getvalue().strip()
        expected = "Loaded from file" \
                   "\nEntry data is checked"
    ...
    def test_save_function_input_2(self):
        with self.captured_output() as (out, err):
            self.__con.load("file validInputData.txt")
            self.__con.validate("")
            self.__con.save("file")
        output = out.getvalue().strip()
        expected = "Loaded from file" \
                   "\nEntry data is checked" \
                   "\nPlease follow save with 'export (file_name)' or 'load (file_name)'"
        self.assertEqual(expected, output)

    def test_display_function_input_1(self):
        self.__con.load("file inputData.txt")
        self.__con.validate("")
        self.__con.save("")
        with self.captured_output() as (out, err):
            self.__con.display("unchecked")
        output = out.getvalue().strip()
    ...
    def test_pickle_function_input_1(self):
        ...
        self.__con.save("")
        with self.captured_output() as (out, err):
            self.__con.pickle("export validInputData")
        output = out.getvalue().strip()
    ...
    def test_pickle_function_input_2(self):
        with self.captured_output() as (out, err):
            self.__con.pickle("load validInputData")
        output = out.getvalue().strip()
